...
const deployOracle: DeployFunction<Settings> = async (
  ...
  const initResult = await WBTCUSDTOracle.at(result.contractInstance.address).transact.init({
    signer
  })
  ...
  const priceFetchResult = await WBTCUSDTOracle.at(result.contractInstance.address).view.price()
